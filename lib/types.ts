...
export type User = {
  ...
  // ライフスタイル（任意・お互い閲覧可）。恋愛マッチングアプリ風の共通点づくり用。
  drinkStatus?: 'yes' | 'sometimes' | 'no';   // お酒：飲む/ときどき/飲まない
  smokeStatus?: 'no' | 'yes' | 'sometimes';   // タバコ：吸わない/吸う/ときどき
  job?: string;                                // 仕事のカテゴリ（会社員 等）
  // 趣味タグ（コミュニティ育成型）。共有台帳 _hobbyTags から選択＋新規追加できる。
  hobbies?: string[];
