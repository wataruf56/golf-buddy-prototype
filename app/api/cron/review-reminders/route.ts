...
// 3日後のレビュー再リマインド。ラウンド完了時に一度「レビューをお願いします」を送るが
// （app/api/rounds/[id]/complete）、3日経ってもレビューしていない人だけに、もう一度だけ
// リマインドを送る。完了済み・未レビュー(pending が残っている)ユーザーのみが対象。
//
...
// 実処理。管理画面などからも呼べるよう関数化。
export async function runReviewFollowup(limit = MAX_PER_TICK): Promise<{ ok: boolean; scanned: number; sent: number; skipped: number }> {
  const now = Date.now();
